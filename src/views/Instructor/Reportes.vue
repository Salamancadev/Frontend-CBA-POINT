<template>
  ...
  <div class="max-w-4xl mx-auto p-6">
    ...
    <div class="flex justify-between mb-4">
      <button @click="clearReport" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded">
        Limpiar Reporte
      </button>
      <button @click="exportCSV" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">
        Exportar CSV
      </button>
    </div>
    ...
    <div v-if="scannedQrs.length > 0" class="overflow-x-auto">
      <table class="table-auto border-collapse border border-gray-400 w-full shadow rounded">
        ...
        <tbody>
          <tr v-for="(item, index) in scannedQrs" :key="index" class="hover:bg-gray-100">
    ...
    <!-- Sin datos -->
    <div v-else class="text-gray-500 text-center mt-6">Aún no se han escaneado códigos QR.</div>
  </div>
</template>
