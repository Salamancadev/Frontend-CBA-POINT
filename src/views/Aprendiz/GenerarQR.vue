  ...
  <div class="min-h-screen py-8 px-2 sm:px-8 text-gray-100" style="background-color: #212529">
    <div class="qr-header mb-10">
      ...
      <p class="text-lg mb-6" style="color: #7ed957">
        Genera tu código QR personal para que el instructor registre tu asistencia
      </p>
    </div>
    ...
    <div class="qr-content">
      ...
      <div class="current-qr-section mb-8">
        <div
          class="rounded-2xl shadow-lg p-6 flex flex-col gap-6"
          ...
          <div class="qr-display-container">
            <div class="qr-visual flex flex-col md:flex-row gap-6 items-center justify-center">
              <div class="qr-code-display">
                <div
                  class="grid grid-cols-7 gap-0.5 rounded-lg p-2 relative w-32 h-36 shadow-inner"
                  style="background-color: #212529; border: 2px solid #7ed957"
                >
                  <div
                    v-for="i in 49"
                    ...
                    :style="
                      getQRPattern(i)
                        ...
                        : 'background-color: #212529; border: 1px solid #7ED957;'
                    "
                    class="w-4 h-4 rounded-sm"
                  ></div>
                  <div
                    class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full px-2 py-1 font-bold shadow text-xs"
              ...
              <div class="qr-details flex flex-col gap-2">
                <div class="student-info mb-2">
                  ...
                  <p class="text-sm">
                    <span class="font-semibold" style="color: #7ed957">Programa:</span>
                    {{ user?.role || 'Biotecnología Agropecuaria' }}
                  </p>
                </div>
                <div
                  class="qr-metadata text-xs rounded-lg p-2"
                  style="background-color: #212529; border: 1px solid #7ed957"
                >
                  <p>
                    <span class="font-semibold" style="color: #7ed957">Código QR:</span>
                    {{ currentQR.codigo }}
                  </p>
                  <p>
                    <span class="font-semibold" style="color: #7ed957">Generado:</span>
                    {{ formatDateTime(currentQR.fechaGeneracion) }}
                  </p>
                  <p>
                    <span class="font-semibold" style="color: #7ed957">Válido hasta:</span>
                    {{ formatDateTime(currentQR.fechaExpiracion) }}
                  </p>
                  <p>
                    <span class="font-semibold" style="color: #7ed957">Ubicación:</span>
                    {{ currentLocation }}
                  </p>
                </div>
              </div>
      ...
      <div class="instructions-section mt-10">
        ...
        <div class="instruction-steps grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="step">
            <div
              class="step-number w-10 h-10 flex items-center justify-center rounded-full font-bold text-xl mb-2"
              style="background-color: #7ed957; color: #212529"
            >
              1
            </div>
            <div class="step-content">
              ...
              <p class="text-sm text-gray-600">
                Tu código QR contiene tus datos personales, fecha, hora y ubicación actual
              </p>
            </div>
          </div>
          ...
          <div class="step">
            ...
            <div class="step-content">
              ...
              <p class="text-sm text-gray-600">
                Presenta tu QR al docente para que lo escanee con su dispositivo
              </p>
            </div>
          </div>
          ...
          <div class="step">
            ...
            <div class="step-content">
              ...
              <p class="text-sm text-gray-600">
                El instructor escanea tu QR y tu asistencia queda automáticamente registrada
              </p>
            </div>
          </div>
        </div>
      ...
      <div class="qr-history-section mt-10">
        ...
        <div class="history-list flex flex-col gap-4">
          <div
            v-for="qr in qrHistory"
            :key="qr.id"
            class="history-item flex items-center rounded-xl shadow p-4 gap-4"
            style="background-color: #212529; border: 1px solid #7ed957"
          >
            <div
              class="history-date flex flex-col items-center justify-center rounded-lg px-3 py-2"
              style="background-color: #7ed957"
            >
              <div class="date-day text-2xl font-bold" style="color: #212529">
                {{ formatDay(qr.fechaGeneracion) }}
              </div>
              <div class="date-month text-xs font-semibold" style="color: #212529">
                {{ formatMonth(qr.fechaGeneracion) }}
              </div>
            </div>
            <div class="history-details flex-1">
              <h4 class="font-bold" style="color: #7ed957">QR de Asistencia</h4>
              <p class="text-sm">
                <span class="font-semibold" style="color: #7ed957">Código:</span> {{ qr.codigo }}
              </p>
              <p class="text-sm">
                <span class="font-semibold" style="color: #7ed957">Hora:</span>
                {{ formatTime(qr.fechaGeneracion) }}
              </p>
              <p class="text-sm">
                <span class="font-semibold" style="color: #7ed957">Estado:</span>
                <span :style="qr.usado ? 'color: #7ED957; font-weight: bold;' : 'color: #fff;'">
                  {{ qr.usado ? 'Usado por instructor' : 'No utilizado' }}
                </span>
              </p>
              <p v-if="qr.instructor" class="text-sm">
                <span class="font-semibold" style="color: #7ed957">Escaneado por:</span>
                {{ qr.instructor }}
              </p>
            </div>
            <div class="history-status flex flex-col items-center">
              <div
                class="status-indicator w-4 h-4 rounded-full mb-1"
                :style="
                  qr.activo
                    ? 'background-color: #7ED957;'
                    : isExpired(qr.fechaExpiracion)
      ...
      <div class="live-feed-section mt-10">
        ...
        <div class="live-feed flex flex-col gap-3">
          <div
            v-for="registro in liveAttendance"
            :key="registro.id"
            class="feed-item flex items-center rounded-xl shadow p-3 gap-4"
            style="background-color: #212529; border: 1px solid #7ed957"
          >
            ...
            <div class="feed-content flex-1">
              ...
              <p class="text-sm" style="color: #fff">
                {{ registro.materia }} - {{ registro.instructor }}
              </p>
              <span class="feed-time text-xs" style="color: #7ed957">{{
          ...
          <div
            v-if="liveAttendance.length === 0"
            class="no-activity text-center py-4"
            style="color: #fff"
          >
    ...
    >
      <div
        class="rounded-xl shadow-lg p-8 w-full max-w-md relative"
        style="background-color: #212529; border: 2px solid #7ed957"
        @click.stop
      >
        <h3 class="text-xl font-bold mb-2" style="color: #7ed957">Permiso de Ubicación</h3>
        <p class="mb-4" style="color: #fff">
          Para generar un QR más preciso, necesitamos acceso a tu ubicación actual.
        </p>
