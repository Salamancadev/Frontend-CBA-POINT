...
>
  ...
  <div>
    <div class="controls">
      <input v-model="searchQuery" id="searchBox" placeholder="Buscar dirección o lugar..." size="40" />
...
export default {
  ...
  mounted() {
    ...
    const puntos = [
      ...
      { nombre: "CBA Auditorio", lat: 4.695316, lng: -74.216070 },
    ...
    const bloques = [
      ...
      { nombre: "CBA Bloque B", lat: 4.695013, lng: -74.216540 },
      { nombre: "CBA Bloque C", lat: 4.694757, lng: -74.216720 },
    ...
    puntos.forEach(p => L.marker([p.lat, p.lng], { icon: redIcon }).addTo(this.map).bindPopup(p.nombre));
    bloques.forEach(p => L.marker([p.lat, p.lng], { icon: yellowIcon }).addTo(this.map).bindPopup(p.nombre));
    internado.forEach(p => L.marker([p.lat, p.lng], { icon: greenIcon }).addTo(this.map).bindPopup(p.nombre));
  },
  methods: {
    async buscarLugar() {
      if (!this.searchQuery) return;
      const resp = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(this.searchQuery)}`, {
      ...
      this.destMarker = L.marker(this.destCoords).addTo(this.map).bindPopup(best.display_name).openPopup();
      this.map.setView(this.destCoords, 15);
    },
    irDestino() {
      if (!this.destCoords) { alert("Primero busca y selecciona un destino"); return; }
      const from = this.userMarker ? this.userMarker.getLatLng() : { lat: 4.6929141, lng: -74.2162249 };
...
.controls { padding: 8px; background: #f7f7f7; }
</style>
